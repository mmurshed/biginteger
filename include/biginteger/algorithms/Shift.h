...
(Zero for B^0, zero for B^1, then 4 at B^2, 3 at B^3, etc.)
  */
  inline std::vector<DataT> ShiftLeft(
      std::vector<DataT> const &bigInt,
      SizeT shift)
  {
    ...
    std::vector<DataT> result(size, 0);
    // Copy
  ...
  inline std::vector<DataT> ShiftRight(const std::vector<DataT> &bigInt, SizeT shift)
  {
    if (shift == 0 || IsZero(bigInt))
      return bigInt;
    ...
    std::vector<DataT> result(newSize, 0);
