...
namespace BigMath
{
    ...
    public:
        static pair<vector<DataT>, vector<DataT>> DivideAndRemainder(const vector<DataT> &a, const vector<DataT> &b, DataT base)
        {
            ...
            int cmp = BigIntegerComparator::Compare(a, b);
            if (cmp < 0)
            {
            ...
            while (BigIntegerComparator::Compare(remainder, b) >= 0)
            {
                ...
                while (d >= 0)
                {
                    ...
                    if (BigIntegerComparator::Compare(shifted_divisor, remainder) <= 0)
                    {
                        break;
                    }
                    d--;
                ...
                for (DataT candidate = base - 1; candidate >= 1; --candidate)
                {
                    ...
                    vector<DataT> product = MultiplicationStrategy::MultiplyUnsigned(shifted_divisor, candidate_vec, base);
                    if (BigIntegerComparator::Compare(product, remainder) <= 0)
                    {
                        q = candidate;
                        break;
                    }
                ...
                vector<DataT> product = MultiplicationStrategy::MultiplyUnsigned(shifted_divisor, q_vec, base);
                remainder = ClassicSubtraction::Subtract(remainder, product, base);
            ...
            BigIntegerUtil::TrimZeros(little_endian_quotient);
