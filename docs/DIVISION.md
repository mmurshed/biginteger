...
|---|---:|---:|---:|
| `a=40 000, b=10 000` | 4.6 | 0.22 | **21 ×** |
| `a=100 000, b=10 000` | 8.9 | 0.45 | **20 ×** |
| `a=200 000, b=50 000` | 22.3 | 1.69 | **14 ×** |
| `a=500 000, b=100 000` | 55 | 4.6 | **12 ×** |
...
| sizes (digits) | early 2026 | current | improvement |
|---|---|---|---|
| 200 000 / 50 000 | 72 × | 14 × | **5.3 ×** |
| 100 000 / 10 000 | 34 × | 20 × | 1.7 × |
| 500 000 / 100 000 | 12 × | 12 × | — (was already at floor) |
| 40 000 / 10 000 | 23 × | 21 × | ~ |

The 200k/50k case improved 5.3× over the session through dispatcher tuning (`NEWTON_SKEW_NUMERATOR` lowered from 4 to 3 after the GMP bench surfaced a boundary-case miss) and the `BurnikelZieglerDivision` odd-divisor shift-normalize change. The other ratios are near their structural floor — the remaining gap to GMP is the same 32-bit-limb / NTT-vs-asm gap that dominates large multiplication.
...
### Port 64-bit hybrid trick to `FastDivision` qhat estimation (cheap, marginal)

The qhat estimation in Knuth Algorithm D is `(acc << 32 | a[i]) / b̂` per quotient digit. The companion subtract-tentative-product step iterates over `b` doing scalar 32×32→64 multiplies (similar shape to `ClassicMultiplication::MultiplyTo`). The same pack-pairs-into-64-bit hybrid trick used in [`MultiplyClassicPtr`](MULTIPLICATION.md#classic-schoolbook) could be applied here.

Estimated win: 1.3–1.7× on FastDivision for medium divisors (1024–8192 limbs). At those sizes the dispatcher already routes to BZ or Newton most of the time, so the impact on end-to-end benchmarks is small (~2–5% on the dispatch corner cases). Effort: ~100 lines, moderate risk (qhat estimation is correctness-sensitive).

### Möller–Granlund 3/2 reciprocal for qhat (cheap, marginal)

[Möller and Granlund, "Improved Division by Invariant Integers" (1994/2010)](https://gmplib.org/~tege/division-paper.pdf) gives a 3/2 reciprocal scheme: precompute a single-limb reciprocal of the normalized divisor's top two limbs, then estimate qhat per digit position with one multiplication and one correction instead of a `__uint128_t / uint64_t` divide. On hardware where `DIV`/`UDIV` is slower than `MUL` (true on most modern microarchitectures), this is a measurable win — roughly 1.5–2× on the qhat estimation alone, which translates to maybe 10–15% on FastDivision overall.

Earlier this was analyzed and pivoted away from in favor of Newton-Raphson, which gave a bigger structural win at the sizes that hurt most. Now that Newton has landed and only the smaller-divisor cases hit FastDivision, the Möller–Granlund win has a smaller surface but is still real. Concretely the gain would show in the dispatcher band `b.size() ∈ (1, NEWTON_MEDIUM_B)` ≈ `(1, 1024)`. Effort: ~80 lines plus dispatcher integration.

### Reduce Newton allocation churn in inner loop

`ApproxReciprocal` allocates several intermediate `vector<DataT>` per iteration. Replacing these with a pre-allocated workspace (mirroring the pattern in `KaratsubaMultiplication`) would cut allocator traffic and potentially help cache behavior. Profile shows allocations are not the top bottleneck (NTT mult is), so the win caps at ~5% on Newton overall.

### Full 64-bit limb refactor (large effort, structural gain on division)

Discussed in detail in [MULTIPLICATION.md §Future opportunities](MULTIPLICATION.md#future-opportunities). For division specifically, this would have **outsized impact** because division's inner loops are scalar (qhat estimation, normalize, subtract-product). Estimated win on the skewed-div benchmarks:

| op | now | est after 64-bit limbs |
|---|---|---|
| div 40k / 10k | 21 × | ~10–12 × |
| div 200k / 50k | 14 × | ~7–9 × |
| div 500k / 100k | 12 × | ~6–8 × |

Cost: 1–2 weeks, 1000–2000 lines across the entire library. Risk: high regression surface from rewriting every carry/borrow chain. Same caveat as for multiplication — NTT-bound operations gain nothing because Goldilocks coefficient capacity is fixed at 16 bits.

### Block-recursive Mulders' short multiplication in Newton iteration

Newton's `R · (2 − D · R)` actually only needs the high half of `D · R` (the high half is what's relevant for the next iteration's precision-doubling). [Mulders (2000), "On Short Multiplications and Divisions"](https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=10.1.1.20.1948) describes computing the high half of a product directly, saving roughly 25–40% of the multiplication cost.

Implementation cost: significant — requires a separate `MulHigh` algorithm that itself splits and recurses, with care about the lost-bits contribution from the discarded low half. Estimated win on Newton iteration: 1.3–1.5×, translating to maybe 10–20% on Newton-bound divisions. Memory pragmatism flags this as not yet worth the complexity given the current Newton implementation already uses full multiplication and the gains have other competing paths.

### Parallel sub-multiplications inside BZ

BZ's `Div3n2n` has independent sub-multiplications that could be parallelized. Similar architectural issues as the multiplication side (header-only library, thread pool management), and BZ already isn't the slowest piece — Newton dominates the skewed-div use case.
...
### Mulders' short multiplication (high half only) — for Newton's reciprocal step

Newton's iteration `R · (2 − D · R)` only needs the high half of `D · R`. Computing only the high half saves work. Not implemented because the Newton implementation already uses full multiplication and Mulders' wins (25–40% on the relevant mult) would be at the constant-factor level rather than asymptotic. Other optimization paths had higher ROI at the time of analysis.
